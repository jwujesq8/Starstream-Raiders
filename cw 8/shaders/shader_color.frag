...
vec3 phongLight(vec3 lightDir, vec3 lightColor, vec3 normal,vec3 viewDir){
	...
	vec3 resultColor = baseColor*diffuse*lightColor+lightColor*specular;
	return resultColor;
}
...
void main()
{
	...
	vec3 normalTex= texture2D(textureNormal, vertexTexCoord).rgb; // = vec3(0,0,1)
	vec3 metallic = texture2D(textureMetallic, vertexTexCoord).rgb + additionalMetallic;
	float roughness = texture2D(textureRoughness, vertexTexCoord).r + additionalRoughness;
